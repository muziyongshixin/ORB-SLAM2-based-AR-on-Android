...
{ 
    mRcw = mTcw.rowRange(0,3).colRange(0,3);    //世界坐标到相机坐标的旋转矩阵
    mRwc = mRcw.t();                            //相机坐标系到世界坐标系的旋转矩阵
    mtcw = mTcw.rowRange(0,3).col(3);           //世界坐标系到相机坐标系的平移矩阵
    mOw = -mRcw.t()*mtcw;                       //相机光心在世界坐标系中的坐标
}
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    ...
    const cv::Mat Pc = mRcw*P+mtcw;             //mappoint坐标转换到当前帧坐标下
    ...
    const float invz = 1.0f/PcZ;                //当前帧坐标转为像素坐标
    ...
    const float maxDistance = pMP->GetMaxDistanceInvariance();      //最高尺度下距离
    const float minDistance = pMP->GetMinDistanceInvariance();      //最低尺度下的距离
    const cv::Mat PO = P-mOw;                                        //光心指向mappoint向量
    const float dist = cv::norm(PO);                                //单位化

    if(dist<minDistance || dist>maxDistance)
        return false;

   // Check viewing angle
    cv::Mat Pn = pMP->GetNormal();                                  //观测到该点的平均方向

    const float viewCos = PO.dot(Pn)/dist;                        //平均方向与观测方向的夹角cos

    if(viewCos<viewingCosLimit)                                     //coslimit = 0.5 大于+-60度就返回false
        return false;

    // Predict scale in the image
    const int nPredictedLevel = pMP->PredictScale(dist,this);       //预测mappoint所在金字塔层

    // Data used by the tracking
    pMP->mbTrackInView = true;                          //该点将被投影
    pMP->mTrackProjX = u;
    pMP->mTrackProjXR = u - mbf*invz;                   //3D投影到双目右侧的横坐标
    pMP->mTrackProjY = v;
...
void Frame::ComputeBoW()
{
    if(mBowVec.empty())
    {
        ...
        mpORBvocabulary->transform(vCurrentDesc,mBowVec,mFeatVec,4);//计算词包mBowVec和mFeatVec，其中mFeatVec记录了属于第i个node（在第4层）的ni个描述子
    }
}
