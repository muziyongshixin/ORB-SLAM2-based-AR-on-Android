...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        float r = RadiusByViewingCos(pMP->mTrackViewCos);       //角度差接近0搜索半径就为2.5，比较大就是4
        ...
        if(bestDist<=TH_HIGH)       //最小距离小于阈值 且最小距离小于次小距离的0.8
        {
...
{
    const vector<MapPoint*> vpMapPointsKF = pKF->GetMapPointMatches();  //提取参考关键帧的mappoint
    ...
    {
        if(KFit->first == Fit->first)   //first为单词的索引，second则对应为该单词索引下的ORB特征点索引集合；
        {
            const vector<unsigned int> vIndicesKF = KFit->second;
            const vector<unsigned int> vIndicesF = Fit->second;

            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)         //遍历属于该node的特征点
            {
                const unsigned int realIdxKF = vIndicesKF[iKF];

                MapPoint* pMP = vpMapPointsKF[realIdxKF];       //取出该特征关键帧中对应的mappoint
                ...
                const cv::Mat &dKF= pKF->mDescriptors.row(realIdxKF);   //提取关键帧中的ORB特征

                int bestDist1=256;  //最小距离
                int bestIdxF =-1 ;
                int bestDist2=256;  //次最小距离
                ...
                for(size_t iF=0; iF<vIndicesF.size(); iF++)
                {
                    ...
                    const cv::Mat &dF = F.mDescriptors.row(realIdxF);       //当前帧中的orb特征

                    const int dist =  DescriptorDistance(dKF,dF);       //求解汉明距离

                    if(dist<bestDist1)              //保留最小距离和次最小距离
                    {
                        bestDist2=bestDist1;
                        bestDist1=dist;
                        bestIdxF=realIdxF;
                ...
                if(bestDist1<=TH_LOW)
                {
                    if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))   //最小距离小于次最小距离的0.7
                    {
                        ...
                        {
                            float rot = kp.angle-F.mvKeys[bestIdxF].angle;  //特征点描述子的角度差,放在直方图rotHist里
                            if(rot<0.0)
                                rot+=360.0f;
        ...
        {
            KFit = vFeatVecKF.lower_bound(Fit->first);      //关键帧中的>=当前帧first的位置
        }
        else
        {
            Fit = F.mFeatVec.lower_bound(KFit->first);      //当前帧中>=关键帧first的位置
        }
    }
    ...
    if(mbCheckOrientation)
    {
        ...
        ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);    //求匹配差角最多的三组

        for(int i=0; i<HISTO_LENGTH; i++)       //去掉除那三组外的匹配的特征点
        {
                             ...
                             const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th)
{
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        cv::Mat p3Dw = pMP->GetWorldPos();      //路标的三维坐标
        cv::Mat p3Dc1 = R1w*p3Dw + t1w;         //路标在当前帧下的坐标
        cv::Mat p3Dc2 = sR21*p3Dc1 + t21;       //当前帧坐标根据sim3算出的旋转平移矩阵转到候选帧下
        ...
        const vector<size_t> vIndices = pKF2->GetFeaturesInArea(u,v,radius);        //找到投影半径内的特征点
        ...
        int bestDist = INT_MAX;
        int bestIdx = -1;
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++) //求投影区域内描述子距离最接近的点
        {
            const size_t idx = *vit;
    ...
    for(int i2=0; i2<N2; i2++)
    {
        ...
        cv::Mat p3Dw = pMP->GetWorldPos();      //路标的世界坐标
        cv::Mat p3Dc2 = R2w*p3Dw + t2w;         //路标在候选帧下的坐标
        cv::Mat p3Dc1 = sR12*p3Dc2 + t12;       //将候选帧下的坐标通过sim3变换至当前帧下
        ...
        int bestDist = INT_MAX;
        int bestIdx = -1;
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++) //遍历投影区域内的特征点，选取最优特征描述点
        {
            const size_t idx = *vit;
